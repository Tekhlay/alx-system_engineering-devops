...
Exercise 4: ls -la === list directory contents in long form, including hidden files which starts with dot(.)

Exercise 5: ls -lna List files digit only
...
Exercise 11: ls -al . .. /boot List all files/directories, including hidden files/directories, from 3 separate directories: current directory, parent of working directory, and /boot directory. The ls command allows multiple directories to be listed separated by spaces.
...
Exercise 16: rm *~ Deletes all files in the current directory that end with a "~" symbol or character
